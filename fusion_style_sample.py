...
def main():
    ...
    def cond_fn(x, t, y=None):
        with th.enable_grad():
            ...
            fixed_y = th.ones((x.shape[0],), dtype=th.long, device=x.device)  # 全是1的类别
            #fixed_y = th.full((x.shape[0],), 10, dtype=th.long, device=x.device)
            logits = classifier(x_in, t)
            log_probs = F.log_softmax(logits, dim=-1)
            selected = log_probs[range(len(logits)), fixed_y.view(-1)]  # 使用固定的 y=1
    ...
    while len(all_images) * args.batch_size < args.num_samples:
        ...
        sample = sample_fn(
            model_fn, # the classes also using for conditional-model's sample
            (args.batch_size, 3, args.image_size, args.image_size),
            clip_denoised=args.clip_denoised,
            model_kwargs=model_kwargs,
            cond_fn=cond_fn,# classifier guidence
